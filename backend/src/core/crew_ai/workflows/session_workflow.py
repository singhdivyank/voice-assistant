...
class SessionWorkflowManager:
    ...
    def advance_to_next_step(self, session_id: str, success_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            next_step = self._determine_next_step(session_id, current_step, success_data)
            ...
            logger.info(f"Session {session_id} advanced: {current_step.value} → {next_step.value}")
            ...
            raise DocJarvisError(f"Workflow advancement failed: {str(e)}")
    
    def update_session_state(self, session_id: str, session_state: SessionState) -> None:
        ...
        try:
            ...
            raise DocJarvisError(f"Session state update failed: {str(e)}")
    ...
    def get_session_progress(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DocJarvisError(f"Session progress retrieval failed: {str(e)}")
    ...
    def mark_session_complete(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DocJarvisError(f"Session completion failed: {str(e)}")
    
    def handle_qa_progress(self, session_id: str, question_index: int, 
                          answered: bool) -> Dict[str, Any]:
        """Handle Q&A progress specifically"""
        try:
            ...
            # Check if we should advance from QA_IN_PROGRESS to QA_COMPLETE
            if (self.get_current_step(session_id) == WorkflowStep.QA_IN_PROGRESS and 
                qa_progress["all_answered"]):
                
                self.advance_to_next_step(session_id, {"qa_completed": True})
            
            return qa_progress
            
        except Exception as e:
            logger.error(f"Failed to handle Q&A progress: {e}")
            raise DocJarvisError(f"Q&A progress handling failed: {str(e)}")
    
    def _determine_next_step(self, session_id: str, current_step: WorkflowStep,
                           success_data: Optional[Dict[str, Any]]) -> WorkflowStep:
        ...
        if current_step == WorkflowStep.QA_IN_PROGRESS:
            ...
            if qa_progress["all_answered"]:
                ...
                return WorkflowStep.QA_IN_PROGRESS  # Stay in Q&A
    ...
    def _calculate_qa_progress(self, session_state: SessionState) -> Dict[str, Any]:
        ...
        answered_count = sum(
            1 for turn in session_state.conversation[:total_questions]
