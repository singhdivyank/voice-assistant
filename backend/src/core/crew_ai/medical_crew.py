...
class MedicalCrew:
    ...
    async def generate_welcome_audio(self, language: str = "en") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to generate welcome audio: %s", str(e))
            raise DocJarvisError(f"Welcome audio generation failed: {str(e)}") from e

    async def process_initial_symptom(
        self, session_state: SessionState, audio_file_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            question_task = Task(
                description=QUESTION_TASK_DESCRIPTION.format(
                    ...
                    initial_complaint=session_state.initial_complaint
                    or "Will be provided from previous tasks",
                ),
            ...
            transcribed_text = (
                _extract_transcription(str(result)) if audio_file_path else None
            )

            questions_user_lang = (
                questions_english
                if session_state.language.lower() == "en"
                else await self._translate_questions_to_user_language(
            ...
            logger.error("Failed to process initial symptom: %s", str(e))
            raise DocJarvisError(f"Initial symptom processing failed: {str(e)}") from e

    async def process_qa_answer(
        self, session_state: SessionState, question_index: int, answer: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            english_answer = (
                answer
                if session_state.language.lower() == "en"
                else await self._translate_text(answer, session_state.language, "en")
            )
            ...
            answered_questions = sum(
                1
                for turn in session_state.conversation[: len(session_state.questions)]
                if turn.answer and turn.answer.strip()
            )
            all_answered = answered_questions >= len(session_state.questions)

            logger.info(
                "Q&A progress: %d/%d for %d",
                answered_questions,
                len(session_state.questions),
                session_state.session_id,
            )
            return {
                "status": "success",
                ...
                "next_step": (
                    "generate_recommendations" if all_answered else "continue_qa"
                ),
            }
        except Exception as e:
            logger.error("Failed to process Q&A answer: %s", str(e))
            raise DocJarvisError(f"Q&A processing failed: {str(e)}") from e

    async def generate_recommednations(
        ...
        try:
            ...
            diagnosis_task = Task(
                ...
                expected_output="Comprehensive medical diagnosis with symptom analysis\
            ...
            medication_task = Task(
                description=MEDICATION_TASK_DESCRIPTION.format(
                    age=session_state.patient_age, gender=session_state.patient_gender
                ),
            ...
            recommendations_usr_lang = (
                recommendations
                if session_state.language.lower() == "en"
                else await self._translate_text(
            ...
            logger.info(
                "Generated medication recommendations for session %d",
                session_state.session_id,
            )
            return {
                "status": "success",
            ...
            logger.error("Failed to generate medication recommendations: %s", str(e))
    ...
    async def generate_and_review_prescriptions(
        ...
        try:
            ...
            raise DocJarvisError(
                f"Prescription generation and review failed: {str(e)}"
            ) from e

    async def process_doctor_response(
        self, review_id: str, email_content: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            crew = Crew(
                agents=[self.agents["prescription_agent"]], tasks=[task], verbose=True
            )
            result = crew.kickoff()

            response_data = _extract_doctor_response(str(result))
            logger.info(
                "Processed doctor response for review %d: %s",
                review_id,
            ...
            logger.error("Failed to process doctor response: %s", str(e))
            raise DocJarvisError(f"Doctor response processing failed: {str(e)}") from e

    async def _translate_questions_to_user_language(
        self, questions: List[str], target_language: str
    ) -> List[str]:
        ...
        for question in questions:
            try:
                ...
                logger.warning(
                    "Translation failed for question, using original: %s", str(e)
                )
    ...
    async def _translate_text(
        ...
        try:
            ...
            crew = Crew(agents=[self.agents["translator"]], tasks=[task], verbose=False)

            result = crew.kickoff()
            return str(result).strip()
        except Exception as e:
            logger.warning("Translation failed, returning original text: %s", str(e))
            return text
