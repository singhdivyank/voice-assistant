"""Module for monitoring performance"""
...
async def get_performance_metrics(
    ...
    try:
        performance_data = (
            monitoring_dashboard.performance_monitor.get_performance_summary()
        )
        if agent_name:
        ...
        performance_data["available_agents"] = list(
            performance_data.get("agents", {}).keys()
        )
        ...
        raise HTTPException(
            status_code=500, detail="Performance metrics unavailable"
        ) from e
...
async def get_cache_statistics():
    ...
    try:
        ...
        cache_stats["agent_configurations"] = (
            monitoring_dashboard.cache_manager.agent_cache_config
        )
        ...
        if hit_rate < 0.3:
            cache_stats["recommendations"].append(
                "Low hit rate detected. Consider increasing cache TTL or reviewing cache keys."
            )

        utilization = cache_stats.get("memory_usage", {}).get("utilization", 0)
        if utilization > 0.9:
            cache_stats["recommendations"].append(
                "High cache utilization. Consider increasing max_size"
            )
        ...
        raise HTTPException(status_code=500, detail="Cache stats unavailable") from e
    ...
    try:
        for agent in AGENTS:
            initial_size = len(
                [
                    k
                    for k in monitoring_dashboard.cache_manager._cache.keys()
        ...
        raise HTTPException(status_code=500, detail="Cache clearing failed") from e
...
async def clear_agent_cache(agent_name: str):
    ...
    if agent_name not in AGENTS:
        raise HTTPException(
            status_code=400, detail=f"Invalid agent name. Must be from: {AGENTS}"
        )
    ...
    try:
        ...
        telemetry.increment_counter(
            "cache_manual_clears", attributes={"agent": agent_name}
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Cache clearing failed for {agent_name}"
        ) from e
...
async def get_load_balancing_stats():
    ...
    try:
        ...
        return {
            ...
            "system_summary": {
                ...
                "total_queued_requests": sum(
                    stats["queue_size"] for stats in load_stats.values()
                ),
            },
            "timestamp": datetime.now().isoformat(),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Load balancing data unavailable"
        ) from e
...
async def get_agent_status():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Agent status unavailable") from e
...
async def get_system_health():
    ...
    try:
        ...
        health_components = {
            ...
            "cache": {
                "status": (
                    "healthy" if cache_stats.get("hit_rate", 0) > 0.3 else "degraded"
                ),
                "hit_rate": cache_stats.get("hit_rate", 0),
            },
            "load_balancer": {
        ...
        if cache_stats.get("hit_rate", 0) < 0.3:
            health_data["recommendations"].append(
                "Consider optimizing cache configuration for better performance"
            )
        ...
        raise HTTPException(status_code=500, detail="System health unavailable") from e
